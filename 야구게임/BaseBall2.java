...
public class BaseBall2 {
	...
	public static int playGame() throws IOException //컴퓨터가 3개의 난수를 발생시키는메소드
	{
		...
		}while((z==x)||(z==y)); // 만일 난수 y가 x 혹은 y와 같은 수일 때 다시 한 번 난수 발생반복
		...
		if(usr[1]==com[2]) ball++;
		if(usr[2]==com[0]) ball++;
		if(usr[2]==com[1]) ball++; //usr, com 두 개의 배열의 데이터 위치가 다른 경우에만 ball처리
		...
		if(args.length==3){ // 게임실행 주어진 3개인 경우, 각각 정수형으로 형변환을 통해x,y,z 에저장
		...
		result= playGame(); // return 값 즉, 시도횟수(count)를 결과값으로 반환하여 result에저장
		}
		System.out.println();
		if(result<=2){
		System.out.println("정답 보신거 아니죠? 아님 미래를 볼줄 아시나?!"); //시도횟수가 2번 이하
		}else if(result<=5){
		System.out.println("와우! 대단해요!");//시도횟수가 3번부터 5번 이하
		}else if(result<=9){
		System.out.println("적당하게 맞추셨네요!"); //시도횟수가 6번~9번
		}else{
		System.out.println("음... 기회가 적은거겠죠?");// 시도횟수가 10번
		}
		}
		}
